...
class ConvertedSFTDataset(Dataset):
    """Dataset for pre-converted SFT data in JSONL format (question/steps/answer)."""
...
class ConvertedSFTDataModule(pl.LightningDataModule):
    """DataModule for pre-converted SFT data. Expects {split}_colar_format.jsonl files."""

    def __init__(self, dataset_name, tiny_dataset=False, epoch_scaling=1, dataset_dir=None, all_config=None):
        super().__init__()
    ...
    def setup(self, stage: str = None):
        def load_split(split: str):
            file_path = self.dataset_dir / f"{split}_colar_format.jsonl"
            data = []
        ...
        if stage == "fit":
            self.train_set = ConvertedSFTDataset(load_split("train"))
            self.val_set = ConvertedSFTDataset(load_split("val"))
        elif stage == "test":
            self.train_set = ConvertedSFTDataset(load_split("train"))
            self.test_set = ConvertedSFTDataset(load_split("test"))
