...
Usage examples:
  ...
  python scripts/convert_ncp_rl_data_chat.py \
    --in /mnt/disk/nrl_ncp/rl_data \
    --out /mnt/disk/baseline_colar/ncp_rl_data_no_implicit \
...
def process_split(tokenizer, in_file: Path, out_file: Path, inplace: bool = False, add_system: bool = False) -> int:
    ...
    with in_file.open("r", encoding="utf-8") as fin, writer_path.open("w", encoding="utf-8") as fout:
        for line in fin:
            ...
            assert len(prompt) > 0
            question = tokenizer.apply_chat_template(prompt,add_generation_prompt=True,tokenize=False)
            answer = ''
            obj = {"question":question, "answer":answer,"idx":str(count),"steps":[]}
...
def main():
    ...
    ap.add_argument("--in", dest="in_dir", default="/mnt/disk/new_nrl_ncp/rl_data", help="Input directory containing train/val/test.jsonl")
    ap.add_argument("--out", dest="out_dir", default="/mnt/disk/baseline_colar/ncp_rl_data_no_implicit", help="Output directory (defaults to <in>_chat if not inplace)")
